...
def convert_doc_to_docx(in_path):
    try:
        ...
        return os.path.join(output_dir, os.path.splitext(os.path.basename(in_path))[0] + '.docx')
    except Exception as e:
        st.error("Failed to convert .doc to .docx. Please save as .docx manually if this persists.")
        return None

# ---- Extract questions as robustly as possible ----
questions = []
if word_file:
    ...
    if st.button("✨ Generate PDF"):
        ...
        for q_num, question in enumerate(questions, 1):
            ...
            if q_num in screenshots_dict:
                try:
                    file = screenshots_dict[q_num]
                    file.seek(0)
                    with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as imf:
                        imf.write(file.read())
                        image_path = imf.name
                    img = PILImage.open(image_path)
                    img_width, img_height = img.size
                    max_width = page_size[0] - inch
                    aspect = img_height / img_width
                    width = min(max_width, 5.5*inch)
                    height = width * aspect
        ...
        def add_page_number(canvas, doc):
            if show_page_numbers:
                ...
                canvas.drawCentredString(page_size[0]/2, 0.40*inch, f"Page {doc.page}")
                canvas.restoreState()
...
2. Optionally upload screenshots, numbered to match question order.
3. Click **Generate PDF**. Each page = one question + screenshot.
4. Download and print or share your task list/PDF.
